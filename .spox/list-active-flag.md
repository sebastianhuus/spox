...
## Acceptance criteria
- [x] `spox list -a` and `spox list --active` both produce output that omits specs in the completed group (status containing "complet", "done", "finish", or "implement") and specs with status `discarded`
- [x] Specs with any other status (e.g. `draft`, `ongoing`) appear normally under their existing group headings
- [x] `spox list` without the flag retains its current behaviour exactly — no regressions
- [x] When all non-completed specs are absent (empty project), the command exits cleanly with no output or a "nothing active" notice
- [x] Help text for `spox list` documents the new flag
