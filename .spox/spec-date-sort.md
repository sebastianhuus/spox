...
## Acceptance criteria
- [ ] `format.md` documents a `date: YYYY-MM-DD` line as the second frontmatter line (after `status:`)
- [ ] The shipped template (`skills/spox/spec-template.md`) includes the `date:` line, and the `new-spec` skill instructs the agent to fill it in with the current date
- [ ] `parse_spec()` ([src/main.rs:90-103](../src/main.rs)) parses the `date:` line into the `Spec` struct; a missing or unparseable date does not error — it sorts as oldest (empty string is fine)
- [ ] `cmd_list()`'s sort ([src/main.rs:841-845](../src/main.rs)) orders by status group, then by date descending (newest first) within the group, then by name as a final tiebreaker
- [ ] Existing specs without a `date:` line continue to list and view correctly (no crash, no required backfill)
